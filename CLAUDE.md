...
- **2026-05-26 — REV 05: reprogramación con observaciones del Ing. Valdés + recorrido a 18-may.** 110 ORIGINAL. Cambios sobre `programa_full.csv` (18 ediciones): BASTMUR30CM (1507374) como arranque sin predecesor; ACEEST01 (917515) FS de BASTMUR30CM; DEM (917553) FS de ACEEST01 (ya no es arranque); ACAMUR02 (851996) y ACAMUR02_1 (917507) reasignados a SS+7d laborables con ACEEST01 (acabados de muros tablaroca/durock temprano); 4 sucesores de ACAMUR02_1 (ACAMUR09/03, ACAPLA03/03-1) reconectados a MURBLOCK01 (1802250) FS+0 para mantener la cadena de acabados al final; luminarias (INSLUM + LUM01..LUM09, 9 conceptos) a 14 días calendario (2 semanas). Luego inicio global recorrido de 25-may → **18-may-2026** (obra empezó lunes pasado). Proceso: backup `110_PRE_REV05_20260526_132227.bak` (obligatorio, MDF prod, sin copia), `DELETE FROM Vinculo` (100→0), `opus-program.ps1` (UPDATE encima + 100 vínculos), `opus-csa.ps1` (recalcula curva mensual), re-sincronización manual de `ProyectoPropuesta` (opus-csa.ps1 no actualiza encabezado → tras CSA queda desync entre MAX(FechaFin) y FechaFinProgramaDeObra; corregir con UPDATE manual). **Verificado:** 107 programados, total = $12,845,065.41 = presupuesto (cierra 100%), ventana **2026-05-18 → 2026-11-03** (~24.1 sem). Backups adicionales: `SANDBOX_PRE_PROGRAMA_20260526_133612.bak`, `SANDBOX_PRE_CSA_20260526_133613.bak` (prefijo genérico del script, son de la 110). Entregable nuevo en `02 ENTREGABLES\REV 05\06 PROGRAMA DE OBRA\Programa de obra - Centro Libanes (REV 05).html` (182 KB). REV 04 conservado para trazabilidad. **Falta validar abriendo OPUS en la 110.**

- **2026-05-19 — Programa replicado en la 110 ORIGINAL.** Tras validar en la copia, se aplicó a la 110 (producción): calendario (34 excepciones, blobs plantilla tomados de la copia vía `opus-calendario.ps1 -PlantillaDb`), programa completo (`opus-program.ps1` + `programa_full.csv`, 107 conceptos + 100 vínculos) y CSA (`opus-csa.ps1`). 110 prístina antes (0 prog/0 vínc), OPUS cerrado. **Verificado:** 107/107 programados, programado = presupuestado = $12,845,065.41 (cierra 100%), ventana 25-may→14-nov-2026. Backups de la 110: `backups/SANDBOX_PRE_{CALENDARIO_215936, PROGRAMA_224157, CSA_224259}.bak` (el prefijo "SANDBOX" es genérico del script; son de la 110). **Falta validar abriendo OPUS en la 110.**
